...
public:
	...
	SOURCE2DEST(GRAPH &graph, DNN &macro);
	~SOURCE2DEST();
...
SOURCE2DEST::SOURCE2DEST(GRAPH &graph, DNN &macro){
	p2NO=graph.p2NO;
	...
	modelNum=macro.numTriphone;
	//printf("%d\n", modelNum);

	slt=new SYMBOLLIST[modelNum];

	for(int i=0;i<modelNum;i++){
		//printf("%d %s %d\n", i, p2MMF[i]->str, strlen(p2MMF[i]->str));
...
void SOURCE2DEST::write2GraphBin(){
	...
	//fwrite(branchNumArray, sizeof(int), nodeNum, out);
...
int main(int argc, char **argv){
	...
	DNN macro;
	macro.readMacroDNN(macroFile);

	GRAPH graph;
	graph.readGraph(graphFile, symFile, 1); // original: 1
