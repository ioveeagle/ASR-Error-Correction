...
int UTILITY::getFileLineCount(char *fileName){
	...
	char line[MAXLEN*3];
	while(fgets(line, MAXLEN*3, in)!=NULL)
		lineCount++;
	fclose(in);
	return lineCount;
}
...
void UTILITY::outputTransition(int s, int e, int in, int out, float weight, SYMBOLLIST *sl, int mType, char *outStr){
	...
	if(weight!=0){
		...
		sprintf(temp, "\t%f", (float)0.4342944819*weight);  // modified by Chi-Yueh Lin @20190731
		
		//sprintf(temp, "\t%f", weight); // Added by Chi-Yueh Lin @20110804
		strcat(msg, temp);
	}
	strcpy(outStr, msg);
