...
WORDTOCI::~WORDTOCI(){
	...
	if(p2k!=NULL)
		delete [] p2k;
	if(nodeHasSilLoop!=NULL)
		delete [] nodeHasSilLoop;
	// graph
	G.N=NULL;
	G.NO=NULL;
	G.p2N=NULL;
...
void WORDTOCI::word2phone(int node, int branch){ 
	...
	for(int i=0;i<variantNum;i++){ // for each pronunication variantion
		...
		if( WFST_SILSP && (nodeHasSilLoop[endNode]==0) ){ 
			char strSp[3]="sp", strSil[4]="sil", strFstr[4]="<F>";

			/*** method 1: self-looping ***/
			/*
			p2k[linkPos]->start=endNode;
			p2k[linkPos]->end=endNode;
			p2k[linkPos]->in=str2Index(strSp, -1);
			p2k[linkPos]->out=epsIndex;
			linkPos++; 
			...
			*/
			...
			/*** method 2: not self-looping ***/
			if( WFST_SILSP == 1 ){
	 			p2k[linkPos]->start=endNode;
				p2k[linkPos]->end=counter;		
				//p2k[linkPos]->end=endNode;
				p2k[linkPos]->in=str2Index(strSp, -1);
				p2k[linkPos]->out=str2Index(strSp, -1);
				//p2k[linkPos]->out=epsIndex;
				...
				p2k[linkPos]->end=endNode;
				p2k[linkPos]->in=str2Index(strFstr, -1);;
				p2k[linkPos]->out=epsIndex;
				p2k[linkPos]->weight=0;
				linkPos++; counter++;
			}
...
int WORDTOCI::computeLinkNum(){
	...
	for(int i=0;i<G.nodeNum;i++){
		...
		for(int j=0;j<G.p2NO[i]->branchNum;j++){
			...
			if( WFST_SILSP==1 ) c+=4; // not self-looping 'sp/sil'  @20120831
			if( WFST_SILSP==2 ) c+=2; // not self-looping 'sil'  @20120831
		}		
	}
	return c; 
}
...
void WORDTOCI::doExpand(char *inFile, char *outFile, char *inDic, char *symFile){
	...
	dic=NULL;
	counter=G.nodeNum;
	nodeHasSilLoop = new int[G.nodeNum];
	for(int i=0; i<G.nodeNum; i++)
		nodeHasSilLoop[i]=0;
...
int main(int argc, char **argv){
	if(argc!=6){
		...
		printf("\topt=0: no word-end sil\n\topt=1: add word-end sil/sp\n\topt=2: add word-end sil only\n");
		exit(1);
	}
        if(atoi(argv[5])==1){ 
		printf("adding sil/sp to word/syllable end (WFST_SILSP)\n");
	}else if(atoi(argv[5])==2){
		printf("adding sil to word/syllable end (WFST_SILSP)\n");
	}
